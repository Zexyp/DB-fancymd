...
CREATE OR REPLACE lokalni_procedura IS
BEGIN
    vzdalena_procedura@muj_db_link(...)
END;
```

> 📌 pokud vzdálená procedura není validní tak se to dozvíme až v době spuštění lokální procedury
...
- <span style="font-size: 1.5em;">`TIMESTAMP`</span> (výchozí)
    ...
    - při změně lokální procedury se lokálně ukládá i časová známka vzdálené procedury
    - nevýhoda &rarr; i když je vzdálená procedura validní, je potřeba překompilovat tu lokální
...
- <span style="font-size: 1.5em">`SIGNATURE`</span>
    ...
    - signatura vzdálené procedury se ukládá v lokální proceduře
